        ...
        response = await client.post(
            ...
            json={
                ...
                "messages": [
                    {
                        "role": "user",
                        "content": f"Пользователь прислал фото места и я предположил: {previous}. Пользователь говорит: '{hint}'. На основе этой подсказки попробуй назвать конкретное место — улицу, район, достопримечательность. Не просто повторяй подсказку, а рассуждай: если это Москва около Арбата — назови конкретные улицы рядом. Отвечай на русском, коротко, начни с 'С учётом подсказки, это скорее всего'."
                    }
                ]
            }
        )
