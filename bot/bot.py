...
async def handle_photo(message: Message):
    ...
    await message.answer("Угадал? Если нет — напиши /hint и расскажи где это было снято 🗺")
...
async def handle_hint(message: Message):
    ...
    user_context[message.from_user.id] = {"waiting_hint": True}
...
async def handle_text(message: Message):
    ...
    negative_words = ["не угадал", "нет", "неверно", "не то", "ошибся", "не правильно", "промахнулся", "такого нет", "не существует"]
    ...
    if ctx.get("waiting_hint"):
        ...
        user_context[message.from_user.id] = {"last_result": location}
        await message.answer(f"📍 С учётом подсказки: {location}", parse_mode="Markdown")
        await message.answer("Теперь угадал? Если нет — снова /hint 🗺")
    ...
    elif ctx.get("celebrated") and any(w in text_lower for w in ["хотя", "погоди", "стоп", "нет", "подожди", "не то"]):
        await message.answer("Ой, подожди! Расскажи подробнее — напиши /hint и уточни 🧭")
        user_context[message.from_user.id] = {"last_result": ctx.get("last_result"), "waiting_hint": True}
